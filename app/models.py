...
class Department(TranslatableModel):
    ...
    img1 = models.ImageField(upload_to='Department/', blank=True, null=True)

    def __str__(self):
        return self.safe_translation_getter('title', any_language=True)

    def save(self, *args, **kwargs):
        creating = self.pk is None  # Yangi obyektmi?
        super().save(*args, **kwargs)  # Avval saqlaymiz, shunda self.id mavjud bo‘ladi

        if not self.slug:
            title = self.safe_translation_getter('title', any_language=True) or "department"
            slug_base = slugify(title)
            self.slug = f'{slug_base}-{self.id}'
            super().save(update_fields=['slug'])  # Faqat slugni yangilab saqlaymiz
